            ...
            if buffer.len() == 3 {
                ...
                if !(64..=126).contains(&last_byte) {
                    ...
                    match last_byte {
                        ...
                        b't' => return parse_csi_cell_size(buffer),
...
/// Vendored patch: recognize cell-size replies and push them to the typed
/// reply sink instead of dropping them as unparsable key codes.
///
/// Recognized forms: `CSI 4 ; height ; width t` (text-area pixel size,
/// response to `CSI 14 t`) and `CSI 6 ; height ; width t` (cell pixel size,
/// response to `CSI 16 t`). Any other `... t` sequence stays unparsable, as
/// upstream.
fn parse_csi_cell_size(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    let s = std::str::from_utf8(&buffer[2..buffer.len() - 1])
        .map_err(|_| could_not_parse_event_error())?;

    let mut split = s.split(';');
    let marker: u16 = next_parsed(&mut split)?;
    let height: u16 = next_parsed(&mut split)?;
    let width: u16 = next_parsed(&mut split)?;
    if split.next().is_some() || !matches!(marker, 4 | 6) {
        return Err(could_not_parse_event_error());
    }

    crate::event::reply::push_cell_size_reply(width, height);
