...
pub struct EventStream {
    ...
    // Vendored patch: the wake worker handle, joined on Drop so that after
    // `EventStream` is dropped no worker can read the tty anymore — the
    // exclusive-ownership guarantee the raw-prefix handoff relies on.
...
impl Default for EventStream {
    fn default() -> Self {
        ...
        // Vendored patch: keep the worker handle for Drop-time join, and
        // check the shutdown flag BEFORE entering another poll_internal —
        // otherwise a queued-but-not-started poll could begin reading the
        // tty after a Drop, defeating the join's quiescence guarantee.
            ...
            while let Ok(task) = receiver.recv() {
                loop {
                    ...
                    match poll_internal(None, &EventFilter) {
                        Ok(true) => break,
                        // Vendored patch: a persistent error (e.g. a latched
                        // reply protocol error) must break the loop instead
                        // of spinning; the stream yields it to the consumer.
                        Err(_) => break,
                        Ok(false) => {}
                    }
        ...
        let result = match poll_internal(Some(Duration::from_secs(0)), &EventFilter) {
            ...
            Ok(false) => {
                    ...
                    .unwrap_or_else(|x| x)
                {
                    ...
                    // Vendored patch: sender is Optional for Drop ordering.
...
impl Drop for EventStream {
    fn drop(&mut self) {
        ...
        // Vendored patch: real quiescence. Drop the task sender first (an
        // idle worker exits its recv), then join the worker: a worker in
        // flight sees the latched wake, returns from poll_internal, observes
        // the shutdown flag (checked before every new poll) and exits. After
        // Drop returns, no worker can read the tty.
