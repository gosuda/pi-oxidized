...
impl InternalEventReader {
    ...
    /// Vendored patch: drive the shared source for reply collection without
    /// touching the public event queues.
    pub(crate) fn probe_try_read(
        &mut self,
        timeout: Option<Duration>,
    ) -> io::Result<Option<InternalEvent>> {
        match self.source.as_mut() {
            Some(source) => source.try_read(timeout),
    ...
    /// Vendored patch: queue an internal event for later delivery (early
    /// ordinary input seen while collecting replies).
    pub(crate) fn queue_event(&mut self, event: InternalEvent) {
        self.events.push_back(event);
    }

    /// Vendored patch: take the first already-queued reply-class internal
    /// event, if any. Non-reply events stay in place, in order.
    #[cfg(unix)]
    pub(crate) fn take_queued_reply(&mut self) -> Option<InternalEvent> {
        let index = self
            .events
            .iter()
            .position(|event| {
    ...
    /// Vendored patch: forward defined recovery from a latched reply
    /// protocol error to the source's parser.
    pub(crate) fn recover_protocol_error(&mut self) -> bool {
        match self.source.as_mut() {
            Some(source) => source.recover_protocol_error(),
            None => false,
        }
    }
}
