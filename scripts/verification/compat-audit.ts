...
import { EXTENSION_COMPAT_REFERENCE_ROOT, assertExtensionCompatReference } from "../reference-identity.ts";
...
export function runCompatAuditWitnesses(repoRoot: string): string[] {
	assertExtensionCompatReference(repoRoot);
	const violations: string[] = [];
	...
	try {
		...
		add("source-evidence", [`${EXTENSION_COMPAT_REFERENCE_ROOT}/packages/ai/src/compat.ts not readable`]);
		return violations;
	}
