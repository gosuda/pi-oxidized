...
fn corrupt_tip(error: serde_json::Error) -> SessionError {
    SessionError::Backend(StorageFailure { code: StorageErrorCode::Corrupt, message: "stored branch tip decode failed".to_owned(), source: Some(Arc::new(error)) })
}

fn value_for<'a>(values: &'a [RawStoredValue], namespace: &str, key: &str) -> Option<&'a RawStoredValue> {
    values.iter().find(|stored| stored.namespace == namespace && stored.key == key)
}

fn tip_values(values: &[RawStoredValue]) -> Result<BranchTips, SessionError> {
    let mut tips = Vec::new();
    let mut seen = HashSet::new();
    for stored in values.iter().filter(|stored| stored.namespace == "pi.branch.tip") {
        if stored.kind != AddressKind::Value {
            return Err(SessionError::Invariant(format!("branch tip {} is not a scalar value", stored.key)));
        }
        if !seen.insert(stored.key.clone()) {
            return Err(SessionError::Invariant(format!("duplicate source branch tip {}", stored.key)));
        }
        let tip = serde_json::from_value::<Option<EntryId>>(stored.value.clone()).map_err(corrupt_tip)?;
...
fn validate_source_snapshot(
    ...
    for stored in &source.values {
        ...
        {
            return Err(SessionError::Invariant(format!("source session branch {} is missing branch.tip", stored.key)));
        }
    }
    ...
    for (branch, tip) in tips {
        ...
        if configuration.is_some() != state.is_some() {
            return Err(SessionError::Invariant(format!("source session branch {branch} has incomplete lane state")));
        }
        if let ForkOptions::Branch { branch: requested, .. } = options
            && requested.as_str() == branch
            && configuration.is_none()
        {
            return Err(SessionError::Invariant(format!("source branch {requested} is not a configured AgentLane")));
        }
        if (source.entries_complete || matches!(options, ForkOptions::Tree { .. }))
            && let Some(tip) = tip
            && !entries.contains_key(tip)
        {
            return Err(SessionError::Invariant(format!("source session branch {branch} has an unknown tip")));
        }
    }
    Ok(())
}
...
fn select_contents(
    ...
    match options {
        ...
        ForkOptions::Branch { branch, entry_id, position, .. } => {
            let source_tip = tips
                .iter()
                .find(|(name, _)| name.as_str() == branch.as_str())
                .map(|(_, tip)| tip.clone())
                .ok_or_else(|| SessionError::Invariant(format!("unknown source branch {branch}")))?;
            ...
            while let Some(current_id) = current {
                if !visited.insert(current_id.clone()) {
                    return Err(SessionError::Invariant("cycle in source branch ancestry".to_owned()));
                }
                let entry = entries.get(&current_id).ok_or_else(|| SessionError::Invariant(format!("corrupt source branch: missing parent {current_id}")))?;
            ...
            if !found {
                ...
                return Err(SessionError::Invariant(format!("fork entry {requested} is not on source branch {branch}")));
            }
...
fn push_value(values: &mut Vec<RawStoredValue>, next_seq: &mut u64, namespace: &str, key: &str, value: serde_json::Value) -> Result<(), SessionError> {
    let seq = *next_seq;
    *next_seq = next_seq.checked_add(1).ok_or_else(|| SessionError::Invariant("sequence overflow".to_owned()))?;
    values.push(RawStoredValue { namespace: namespace.to_owned(), key: key.to_owned(), kind: AddressKind::Value, value, seq });
    Ok(())
}
...
fn copy_disposition(namespace: &str, key: &str, scope: ForkScope, selected: &HashSet<EntryId>) -> Result<bool, SessionError> {
    if namespace == "pi.session.name" {
        return Ok(true);
    }
    if namespace == "pi.entry.label" {
        return Ok(matches!(scope, ForkScope::Tree) || selected.contains(&EntryId::from(key.to_owned())));
    }
    if namespace == "pi.branch.tip" || namespace == "pi.lane.config" || namespace == "pi.lane.state" {
        return Ok(false);
    }
    if namespace == "pi.result" || namespace.starts_with("pi.op.") || namespace.starts_with("pi.pending.") {
        return Ok(false);
    }
    if namespace == "pi" || namespace.starts_with("pi.") {
        return Err(SessionError::Invariant(format!("unknown reserved fork namespace {namespace}")));
    }
...
pub fn create_fork_snapshot(source: &ForkSourceSnapshot, options: &ForkOptions) -> Result<ForkDestinationSnapshot, SessionError> {
    if let ForkOptions::Branch { branch, .. } = options {
        validate_branch_name(branch)?;
    }
    let mut source_entries = HashMap::with_capacity(source.entries.len());
    for entry in &source.entries {
        if source_entries.insert(entry.id().clone(), entry.clone()).is_some() {
            return Err(SessionError::Invariant(format!("duplicate source entry id {}", entry.id())));
        }
    }
    ...
    let mut entries: Vec<Entry> = selected.iter().filter_map(|id| source_entries.get(id).cloned()).collect();
    entries.sort_by_key(Entry::seq);
    let mut seen_sequences = HashSet::new();
    for entry in &entries {
        if !seen_sequences.insert(entry.seq()) {
            return Err(SessionError::Invariant(format!("duplicate source entry sequence {}", entry.seq())));
        }
    }
    ...
    for (branch, tip) in destination_tips {
        let tip_value = serde_json::to_value(&tip).map_err(|error| SessionError::Invariant(format!("branch tip serialization failed: {error}")))?;
        push_value(&mut values, &mut next_seq, "pi.branch.tip", &branch, tip_value)?;
        if let Some(configuration) = value_for(&source.values, "pi.lane.config", &branch) {
            values.push(RawStoredValue { namespace: "pi.lane.config".to_owned(), key: branch.clone(), kind: AddressKind::Value, value: configuration.value.clone(), seq: next_seq });
            next_seq = next_seq.checked_add(1).ok_or_else(|| SessionError::Invariant("sequence overflow".to_owned()))?;
            let state = serde_json::to_value(LaneState::default()).map_err(|error| SessionError::Invariant(format!("lane state serialization failed: {error}")))?;
    ...
    for stored in &source.values {
        if copy_disposition(&stored.namespace, &stored.key, scope, &selected)? {
            push_value(&mut values, &mut next_seq, &stored.namespace, &stored.key, stored.value.clone())?;
        }
    }

    Ok(ForkDestinationSnapshot { entries, values, next_seq })
}
...
pub fn fork_snapshot_writes(snapshot: &ForkDestinationSnapshot) -> Vec<CommittedWrite> {
    ...
    writes.extend(snapshot.values.iter().cloned().map(|stored| {
        CommittedWrite::Value(CommittedValueWrite::Set { seq: stored.seq, namespace: stored.namespace, key: stored.key, value: stored.value })
    }));
