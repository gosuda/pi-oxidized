...
use super::{BranchScan, CommitResult, Entry, EntryId, EntryQuery, EntryScan, EntryStructure, LaneName, ListElement, ListReadOptions, RawAddress, RawListElement, RawStoredValue, SessionError, SessionStats, StorageBranchScan, UsageRow, UsageScan, Value, ValueList, StoredValue, Write};
...
pub trait SessionReaderExt: SessionReader {
    ...
    fn get_value<'a, T: DeserializeOwned + Send + 'a>(&'a self, address: &'a Value<T>, cx: &'a Context) -> BoxFuture<'a, Result<Option<StoredValue<T>>, SessionError>> {
        Box::pin(async move {
            let Some(raw) = self.get_value_json(&address.erase(), cx).await? else { return Ok(None); };
            let value = serde_json::from_value(raw.value).map_err(corrupt_value)?;
            Ok(Some(StoredValue { address: address.clone(), value, seq: raw.seq }))
        })
    }
    ...
    /// through.
    fn scan_values<'a, T: DeserializeOwned + Send + 'a>(&'a self, prefix: &'a Value<T>, cx: &'a Context) -> BoxFuture<'a, Result<Vec<StoredValue<T>>, SessionError>> {
        Box::pin(async move {
            let values = self.scan_values_json(&prefix.erase(), cx).await?;
            values.into_iter().map(|raw| {
                let value = serde_json::from_value(raw.value).map_err(corrupt_value)?;
                let address = Value::new(prefix.static_namespace(), raw.key).map_err(|_| corrupt_message("stored address is invalid"))?;
                Ok(StoredValue { address, value, seq: raw.seq })
            }).collect()
        })
    }
    ...
    fn read_list<'a, T: DeserializeOwned + Send + 'a>(&'a self, address: &'a ValueList<T>, options: Option<ListReadOptions>, cx: &'a Context) -> BoxFuture<'a, Result<Vec<ListElement<T>>, SessionError>> {
        Box::pin(async move {
            self.read_list_json(&address.erase(), options, cx).await?.into_iter().map(|raw| {
                serde_json::from_value(raw.value).map(|value| ListElement { seq: raw.seq, value }).map_err(corrupt_value)
            }).collect()
        })
    }
}
fn corrupt_value(error: serde_json::Error) -> SessionError {
    SessionError::Backend(StorageFailure { code: StorageErrorCode::Corrupt, message: "stored value decode failed".to_owned(), source: Some(Arc::new(error)) })
}
...
pub trait Branch: Send + Sync {
    ...
    fn find_entries<'a>(&'a self, q: Option<&'a BranchScan>, cx: &'a Context) -> BoxFuture<'a, Result<Vec<Entry>, SessionError>>;
    /// The first entry [`find_entries`](Self::find_entries) returns, if any.
    fn find_entry<'a>(&'a self, q: Option<&'a BranchScan>, cx: &'a Context) -> BoxFuture<'a, Result<Option<Entry>, SessionError>>;
...
pub trait Session: SessionReader {
    ...
    fn find_entries<'a>(&'a self, q: Option<&'a EntryQuery>, cx: &'a Context) -> BoxFuture<'a, Result<Vec<Entry>, SessionError>>;
    /// The first entry [`find_entries`](Self::find_entries) returns, if any.
    fn find_entry<'a>(&'a self, q: Option<&'a EntryQuery>, cx: &'a Context) -> BoxFuture<'a, Result<Option<Entry>, SessionError>>;
...
pub trait SessionRepo: Send + Sync {
    ...
    fn list<'a>(&'a self, options: Option<Self::ListOptions>, cx: &'a Context) -> BoxFuture<'a, Result<Vec<Self::Metadata>, SessionError>>;
...
pub struct SessionMetadata {
    ...
    #[serde(rename = "storageVersion")] pub storage_version: u32,
