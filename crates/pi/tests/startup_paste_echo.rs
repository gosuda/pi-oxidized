...
fn paste_after_first_frame_paints_label_as_synchronized_text() {
	...
	env.insert("PI_CODING_AGENT_DIR".to_owned(), agent.to_string_lossy().into_owned());
	env.insert(
	...
	let frame_policy =
		SettlePolicy::new(Duration::from_millis(80), Duration::from_secs(20)).expect("settle policy");
	session
		.read_output(&frame_policy, |bytes| {
			let begin = bytes
				.windows(SYNC_BEGIN.len())
				.any(|window| window == SYNC_BEGIN);
			let end = bytes.windows(SYNC_END.len()).any(|window| window == SYNC_END);
			begin && end
		})
	...
	let paint_policy =
		SettlePolicy::new(Duration::from_millis(300), Duration::from_secs(2)).expect("settle policy");
