...
pub(crate) fn object<'a>(value: &'a JsonValue, description: &str) -> Result<&'a JsObject, ServiceError> {
    value.as_object().ok_or_else(|| invalid(description))
}

pub(crate) fn array<'a>(value: &'a JsonValue, description: &str) -> Result<&'a [JsonValue], ServiceError> {
    value.as_array().map(Vec::as_slice).ok_or_else(|| invalid(description))
}
...
pub(crate) fn string(value: &JsonValue, description: &str) -> Result<String, ServiceError> {
    ...
    value.try_to_utf8().map_err(|error| invalid(format!("{description}: {error}")))
}

pub(crate) fn nullable_string(value: &JsonValue, description: &str) -> Result<Option<String>, ServiceError> {
    if value.is_null() {
        Ok(None)
    } else {
...
pub(crate) fn nullable<T>(value: &JsonValue, decode: impl FnOnce(&JsonValue) -> Result<T, ServiceError>) -> Result<Option<T>, ServiceError> {
    if value.is_null() {
        Ok(None)
    } else {
...
pub(crate) fn strings(values: &[JsonValue], description: &str) -> Result<Vec<String>, ServiceError> {
    values.iter().map(|value| string(value, description)).collect()
}

pub(crate) fn json_object(fields: impl IntoIterator<Item = (&'static str, JsonValue)>) -> JsonValue {
...
pub(crate) fn serde_decode<T: DeserializeOwned>(value: &JsonValue, description: &str) -> Result<T, ServiceError> {
...
pub(crate) fn serde_encode<T: Serialize>(value: &T, description: &str) -> Result<JsonValue, ServiceError> {
    let value = serde_json::to_value(value).map_err(|error| invalid(format!("{description}: {error}")))?;
