...
use pi_agent::service::{delta::DeltaOp, value::JsonValue, wire::{ServiceCall, ServiceProviderUpdate}};
...
pub type PublishUpdate = Arc<dyn Fn(String, ServiceProviderUpdate<DeltaOp>, Context) -> BoxFuture<'static, ()> + Send + Sync>;
...
pub trait RoutedSessionAttachment: Send + Sync {
    ...
    fn invoke_service(&self, call: ServiceCall, publish: PublishUpdate, cx: Context) -> BoxFuture<'_, Result<Option<JsonValue>, HostError>>;
...
pub trait RoutedServerPresentation: Send + Sync {
    ...
    fn attach_session(&self, session_id: String, cx: Context) -> BoxFuture<'_, Result<(), HostError>>;
    ...
    fn prepare_session_removal(&self, session_id: String, cx: Context) -> BoxFuture<'_, Result<(), HostError>>;
}
...
pub trait RoutedServerServiceHost: Send + Sync {
    ...
    fn attach_client(&self, presentation: Arc<dyn RoutedServerPresentation>, cx: Context) -> BoxFuture<'_, Result<Arc<dyn RoutedServerServiceAttachment>, HostError>>;
}
...
pub trait RoutedSessionHandle: Send + Sync {
    ...
    fn attach_client(&self, cx: Context) -> BoxFuture<'_, Result<Arc<dyn RoutedSessionAttachment>, HostError>>;
...
pub trait ServerHost: Send + Sync + 'static {
    ...
    fn resolve_session(&self, session_id: String, cx: Context) -> BoxFuture<'_, Result<Self::Metadata, HostError>>;
    /// Opens the exact record returned by resolution.
    fn open_session(&self, metadata: Self::Metadata, cx: Context) -> BoxFuture<'_, Result<Arc<dyn RoutedSessionHandle>, HostError>>;
}
