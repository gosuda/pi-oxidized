...
mod tests {
    ...
    async fn attach(client: &PiClient, server: &TestServer, session_id: &str) -> PiSessionHandle {
        let (handle, (id, command)) =
            tokio::join!(client.attach_session(session_id), server.core.next_request());
    ...
    async fn lease_conflicts_are_typed_ownership_errors() {
        ...
        let (exclusive, (attach_id, attach_command)) = tokio::join!(
            client.acquire_session("session-1", SessionLeaseMode::Exclusive),
            server.core.next_request()
        );
    ...
    async fn detach_leases(handles: &[PiSessionHandle], server: &TestServer, labels: &[(&str, &str)]) {
        for (index, handle) in handles.iter().enumerate() {
            let (outcome, (id, command)) = tokio::join!(handle.detach(), server.core.next_request());
    ...
    async fn shared_lease_detaches_only_after_final_release() {
        ...
        let (outcome, (id, command)) = tokio::join!(second.detach(), server.core.next_request());
    ...
    async fn detach_is_not_disconnect_and_reattach_works() {
        ...
        let (outcome, (id, _command)) = tokio::join!(handle.detach(), server.core.next_request());
        server.respond(&id, detach_result("session-1")).await;
        outcome.expect("detach");
    ...
    async fn transport_failure_table_maps_to_exactly_one_variant() {
        ...
        // → Ownership.
        let (attaching, (id, command)) =
            tokio::join!(client.attach_session("locked-session"), server.core.next_request());
        ...
        let (listing, (id, command)) = tokio::join!(client.list_sessions(), server.core.next_request());
    ...
    async fn request_ids_are_monotonic() {
        let (client, server) = connect_scripted().await;
        let (listed, attached, (first_id, first_command), (second_id, second_command)) =
            futures::join!(
                client.list_sessions(),
                client.attach_session("session-1"),
                server.core.next_request(),
                server.core.next_request()
            );
    ...
    async fn coalesced_out_of_order_responses_correlate() {
        let (client, server) = connect_scripted().await;
        let (listed, attached, (attach_id, attach_command), (list_id, list_command)) =
            futures::join!(
                client.list_sessions(),
                client.attach_session("session-1"),
                server.core.next_request(),
    ...
    async fn mismatched_response_command_fails_the_connection() {
        let (client, server) = connect_scripted().await;
        let (listed, (id, command)) =
            tokio::join!(client.list_sessions(), server.core.next_request());
        assert!(matches!(command, Command::List));
    ...
    async fn detach_failure_restores_active_lease_and_retry_succeeds() {
        let (client, server) = connect_scripted().await;
        let (acquired, (attach_id, attach_command)) = tokio::join!(
            client.acquire_session("session-1", SessionLeaseMode::Exclusive),
            server.core.next_request()
        );
        assert!(matches!(attach_command, Command::Attach { .. }));
        ...
        let (first_detach, (detach_id, detach_command)) =
            tokio::join!(handle.detach(), server.core.next_request());
        ...
        let (second_detach, (retry_id, retry_command)) =
            tokio::join!(handle.detach(), server.core.next_request());
    ...
    async fn reacquisition_serializes_behind_final_detachment() {
        ...
        let (detaching, (detach_id, detach_command)) =
            tokio::join!(first.detach(), server.core.next_request());
        assert!(matches!(detach_command, Command::Detach { .. }));
        ...
        detaching.expect("detach");

        let (second, (second_attach_id, second_attach_command)) =
            tokio::join!(reacquiring, server.core.next_request());
    ...
    async fn create_session_takes_exclusive_lease() {
        let (client, server) = connect_scripted().await;
        let (creating, (id, command)) =
            tokio::join!(client.create_session(CreateSessionOptions::default()), server.core.next_request());
    ...
    async fn lower_revision_is_accepted_after_reacquire() {
        ...
        let (outcome, (id, _command)) = tokio::join!(first.detach(), server.core.next_request());
        server.respond(&id, detach_result("session-1")).await;
        outcome.expect("detach");
        ...
        let (reacquiring, (attach_id, attach_command)) =
            tokio::join!(client.attach_session("session-1"), server.core.next_request());
        assert!(matches!(attach_command, Command::Attach { .. }));
