...
mod tests {
    ...
    fn make_runtime_with_captured_writer() -> Result<CapturedRuntime, String> {
        let writer = SharedWriter::new();
        let captured = writer.clone();
        let tui = Tui::new(
            writer,
    ...
    fn sanitize_title_caps_utf8_bytes_without_splitting_multibyte() {
        ...
        let input = format!("{near_cap}中");
        let sanitized = sanitize_title(&input);
        assert_eq!(sanitized.len(), 255);
        assert_eq!(sanitized, near_cap);
        assert_eq!(&input[sanitized.len()..], "中"); // next scalar absent, not split
    }

    #[tokio::test]
    async fn extension_title_sink_strips_injection_and_wraps_in_fixed_osc() -> Result<(), String> {
    ...
    #[tokio::test]
    async fn extension_title_sink_caps_payload_at_256_bytes() -> Result<(), String> {
        let (mut rt, _log, captured) = make_runtime_with_captured_writer()?;
        let before = captured.snapshot();
    ...
    #[tokio::test]
    async fn extension_title_sink_none_emits_empty_payload() -> Result<(), String> {
        let (mut rt, _log, captured) = make_runtime_with_captured_writer()?;
        let before = captured.snapshot();
    ...
    #[tokio::test]
    async fn extension_title_sink_c1_controls_removed() -> Result<(), String> {
        let (mut rt, _log, captured) = make_runtime_with_captured_writer()?;
        let before = captured.snapshot();
