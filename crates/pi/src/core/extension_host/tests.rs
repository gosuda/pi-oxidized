...
fn assert_host_missed_methods(host: &FakeHost, methods: &[&str]) -> R {
    let requests = host.requests.lock().map_err(|_| "request lock poisoned")?;
    assert!(
        !requests
            .iter()
            .any(|frame| methods.contains(&frame.method.as_str())),
        "host unexpectedly received one of {methods:?}"
    );
    Ok(())
}

/// Build a runner whose two endpoints collide in every aggregate registry.
async fn make_registry_collision_runner()
-> Result<(Arc<HostExtensionRunner>, Vec<FakeHost>), BoxErr> {
...
/// Assert the collision diagnostics for the registry collision fixture.
fn assert_registry_collision_diagnostics(runner: &HostExtensionRunner) {
...
async fn aggregate_registry_and_command_collisions_are_first_wins() -> R {
    let (runner, hosts) = make_registry_collision_runner().await?;
    assert_eq!(runner.get_registered_commands(), vec!["same", "only-b"]);
    assert_registry_collision_diagnostics(&runner);

    assert!(runner.execute_command("same", "args").await?);
    hosts[0].wait_for_request("command.execute").await?;
    assert_host_missed_methods(&hosts[1], &["command.execute"])?;
    ...
    hosts[1]
        ...
        .await?;
    assert_host_missed_methods(&hosts[0], &[pi_ext::protocol::SHORTCUT_EXECUTE_METHOD])?;
    ...
    // serving ctrl+x.
    assert!(runner.execute_shortcut("ctrl+x").await?.handled);
    assert!(
        runner
    ...
    assert_host_missed_methods(&hosts[1], &["command.execute", "tool.renderHtml"])?;
    runner.shutdown_once().await;
    Ok(())
}
...
async fn failed_restart_prepare_keeps_old_runner_provider_and_transport_live() -> R {
    ...
    assert!(!methods.iter().any(|method| method == "session_shutdown"));
    assert!(!methods.iter().any(|method| method == "shutdown"));
    runner.shutdown_once().await;
    Ok(())
}
