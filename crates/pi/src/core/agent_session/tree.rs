...
mod tests {
    ...
    async fn get_last_assistant_text_skips_aborted_empty() -> TestResult {
        ...
        session
            .agent
            .push_message(AgentMessage::Llm(Box::new(Message::Assistant(Box::new(aborted)))));
        session
            .agent
            .push_message(AgentMessage::Llm(Box::new(Message::Assistant(Box::new(good)))));
    ...
    async fn append_export_messages(session: &AgentSession) -> TestResult {
        ...
        sm.append_message(&AgentMessage::Llm(Box::new(Message::Assistant(Box::new(assistant)))))?;
        Ok(())
    }
