...
mod tests {
    ...
    async fn failed_restart_prepare_keeps_old_runner_provider_and_transport_live() -> TestResult {
        ...
        config.model_runtime = Some(Arc::new(
            ModelRuntime::create(CreateModelRuntimeOptions::default()).await?,
        ));
        let session = AgentSession::new(config)?;
        ...
        let error = session
            ...
            .await
            .err()
            .ok_or_else(|| io::Error::other("prepare must reject number flag"))?;
    ...
    async fn reload_returns_structured_host_diagnostics() -> TestResult {
        ...
        assert!(
            diagnostics[0].message.contains("No such file or directory"),
