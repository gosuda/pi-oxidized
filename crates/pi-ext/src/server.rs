...
//
// Mode-3 producer DTOs for `extensions.load`. Kept local to this module so
// current `pi-ext` protocol / `pi` `RegistrySnapshotWire` ownership stays
...
mod tests {
    ...
    impl DuplexFixture {
        ...
        /// handshake, and establish a default test context.
        async fn connect() -> Result<Self, Box<dyn Error>> {
            ...
            // Compat handshake is fine here: server ACKs HelloAck::local()
            // with the same compiled COMPATIBILITY_VERSION the client expects.
            // (Native validate_hello ignores inbound compatibilityVersion.)
    ...
    async fn cancel_token_for_matching_kind_is_cancelled_after_terminal() -> R {
        // Drive cancel over the raw JSONL wire rather than HostClient::cancel:
        // current client cancel retention can drop the terminal oneshot before
        // the cancelled error frame is delivered, which is outside this
        // server's contract. The server must still cancel the extension token
        // and answer a correlated `cancelled` error frame.
        let (ext, handles) = DemoExtension::new();
        let (mut client, server) = spawn_raw(ext, ServerConfig::default());
        client.hello(PROTOCOL_VERSION, "anything").await?;
        let _ack = client.recv().await?;
        ...
        client
            ...
            .await?;
        let cancelled_terminal = client.recv().await?;
        assert_eq!(cancelled_terminal.id, 2);
        assert_eq!(cancelled_terminal.kind, FrameKind::Error);
        assert_eq!(cancelled_terminal.payload["code"], "cancelled");
        assert!(
            token.is_cancelled(),
