...
describe("extension theme API", () => {
	...
	test("rapid theme updates coalesce paused factory recreations to the newest theme", async () => {
		const pending: Array<{
			theme: string;
			resolve: () => void;
		...
		const { promise: finalFactoryStarted, resolve: resolveFinalFactoryStarted } = Promise.withResolvers<void>();
		const raceFactory: ExtensionFactory = (pi) => {
			pi.registerCommand("theme-race", {
				description: "Creates an asynchronous overlay",
				async handler(_args, ctx) {
					customResults.push(await ctx.ui.custom((_tui, theme, _keybindings, done) => {
						...
						if (pending.length === 2) resolveFinalFactoryStarted();
						return promise;
					}));
				},
			});
		...
		(host as unknown as { applyThemeUpdate(update: unknown): void }).applyThemeUpdate(themeUpdateFrame("green").payload);
		(host as unknown as { applyThemeUpdate(update: unknown): void }).applyThemeUpdate(themeUpdateFrame("blue").payload);
		expect(pending.map((entry) => entry.theme)).toEqual(["dark"]);

		pending[0]?.resolve();
		await finalFactoryStarted;
		expect(pending.map((entry) => entry.theme)).toEqual(["dark", "blue"]);
		...
		pending[1]?.done("complete");
		await collector.awaitFrame((frame) => frame.id === 62 && frame.kind === "res");
		pending[1]?.done("ignored");
		expect(customResults).toEqual(["complete"]);
		expect(collector.frames.filter((frame) => frame.id === 62 && frame.kind === "res")).toHaveLength(1);
	...
	test("coalesced newest factory rejection reports once and retains the resolved component", async () => {
		...
		const disposed: string[] = [];
		const { promise: initialFactoryStarted, resolve: resolveInitialFactoryStarted } = Promise.withResolvers<void>();
		const { promise: recreationStarted, resolve: resolveRecreationStarted } = Promise.withResolvers<void>();
		const { promise: latestFactoryStarted, resolve: resolveLatestFactoryStarted } = Promise.withResolvers<void>();
		const raceFactory: ExtensionFactory = (pi) => {
			pi.registerCommand("theme-failure-race", {
				description: "Creates an asynchronous overlay that can reject during recreation",
				async handler(_args, ctx) {
					await ctx.ui.custom((_tui, theme, _keybindings, done) => {
						...
						const { promise, resolve, reject } = Promise.withResolvers<typeof component>();
						pending.push({
							theme: name,
						...
						if (pending.length === 1) resolveInitialFactoryStarted();
						if (pending.length === 2) resolveRecreationStarted();
						if (pending.length === 3) resolveLatestFactoryStarted();
						return promise;
					});
				},
			});
		...
		const { collector, stdin, host, runPromise } = await connectHost([raceFactory]);
		stdin.push(Buffer.from(encodeFrameString({
			id: 64, kind: "req", method: "command.execute", payload: { command: "theme-failure-race", args: "" },
		})));
		await initialFactoryStarted;
		pending[0]?.resolve();
		const initial = await collector.awaitFrame((frame) => frame.method === "uiSlot");
		const initialKey = (initial.payload as Record<string, unknown>)["key"] as string;

		(host as unknown as { applyThemeUpdate(update: unknown): void }).applyThemeUpdate(themeUpdateFrame("red").payload);
		await recreationStarted;
		(host as unknown as { applyThemeUpdate(update: unknown): void }).applyThemeUpdate(themeUpdateFrame("green").payload);
		(host as unknown as { applyThemeUpdate(update: unknown): void }).applyThemeUpdate(themeUpdateFrame("blue").payload);
		expect(pending.map((entry) => entry.theme)).toEqual(["dark", "red"]);

		pending[1]?.resolve();
		await latestFactoryStarted;
		expect(pending.map((entry) => entry.theme)).toEqual(["dark", "red", "blue"]);
		pending[2]?.reject(new Error("latest theme factory failed"));
		const error = await collector.awaitFrame((frame) =>
			frame.method === "extensionError"
			&& String((frame.payload as Record<string, unknown>)["message"]).includes("latest theme factory failed"),
		);
		expect((error.payload as Record<string, unknown>)["code"]).toBe("extension_error");
		expect(collector.frames.filter((frame) => frame.method === "extensionError")).toHaveLength(1);
		expect(disposed).toEqual(["red"]);
		expect(collector.frames.filter((frame) => frame.method === "uiSlot")).toHaveLength(1);

		stdin.push(Buffer.from(encodeFrameString({
			id: 65, kind: "req", method: "render", payload: { key: initialKey, width: 80 },
		})));
		const rendered = await collector.awaitFrame((frame) => frame.id === 65 && frame.kind === "res");
		expect((rendered.payload as Record<string, unknown>)["runs"])
			.toEqual((initial.payload as Record<string, unknown>)["runs"]);

		pending[0]?.done("complete");
		await collector.awaitFrame((frame) => frame.id === 64 && frame.kind === "res");
		stdin.push(null);
		host.dispose("test");
		await runPromise.catch(() => void 0);
	});

	test("initial async custom factory transfers failure completion through coalesced theme updates", async () => {
		const pending: Array<{
			theme: string;
			resolve: () => void;
			reject: (reason: Error) => void;
		...
		const { promise: latestFactoryStarted, resolve: resolveLatestFactoryStarted } = Promise.withResolvers<void>();
		const raceFactory: ExtensionFactory = (pi) => {
			pi.registerCommand("initial-theme-failure-race", {
				description: "Starts an initial async overlay that is superseded before installation",
				async handler(_args, ctx) {
					customResults.push(await ctx.ui.custom((_tui, theme) => {
						const name = String((theme as { name?: unknown }).name ?? "dark");
						const component = { render: () => [`overlay-${name}`] };
						const { promise, resolve, reject } = Promise.withResolvers<typeof component>();
						pending.push({ theme: name, resolve: () => resolve(component), reject });
						if (pending.length === 1) resolveInitialFactoryStarted();
						if (pending.length === 2) resolveLatestFactoryStarted();
						return promise;
					}));
				},
			});
		...
		const { collector, stdin, host, runPromise } = await connectHost([raceFactory]);
		stdin.push(Buffer.from(encodeFrameString({
			id: 66, kind: "req", method: "command.execute",
			payload: { command: "initial-theme-failure-race", args: "" },
		})));
		await initialFactoryStarted;
		(host as unknown as { applyThemeUpdate(update: unknown): void }).applyThemeUpdate(themeUpdateFrame("red").payload);
		(host as unknown as { applyThemeUpdate(update: unknown): void }).applyThemeUpdate(themeUpdateFrame("green").payload);
		...
		await latestFactoryStarted;
		expect(pending.map((entry) => entry.theme)).toEqual(["dark", "blue"]);
		pending[1]?.reject(new Error("latest initial factory failed"));

		const error = await collector.awaitFrame((frame) =>
			frame.method === "extensionError"
			&& String((frame.payload as Record<string, unknown>)["message"]).includes("latest initial factory failed"),
		);
		expect((error.payload as Record<string, unknown>)["code"]).toBe("extension_error");
		const command = await collector.awaitFrame((frame) => frame.id === 66 && frame.kind === "res");
		expect(command.payload).toEqual({ ok: true });
		expect(customResults).toEqual([undefined]);
		expect(collector.frames.filter((frame) => frame.method === "extensionError")).toHaveLength(1);
		expect(collector.frames.filter((frame) => frame.id === 66 && frame.kind === "res")).toHaveLength(1);
