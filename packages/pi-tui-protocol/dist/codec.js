import { isMethod, MAX_FRAME_BYTES, } from "./types.js";
 ...
 */
export function validateFrame(frame, requireAllowlisted = false) {
...
function validateUiSlotPayload(payload) {
...
function validateHyperlink(value) {
...
function validateOverlayOptions(value) {
...
function validateCellCount(value, field) {
...
export function requestFrame(id, method, payload = {}) {
    return { id, kind: "req", method, payload };
}
/** Build a typed response frame. */
export function responseFrame(id, method, payload = {}) {
    return { id, kind: "res", method, payload };
}
/** Build a typed event frame. */
export function eventFrame(id, method, payload = {}) {
    return { id, kind: "event", method, payload };
}
/** Build an error frame. */
export function errorFrame(id, method, error) {
    const payload = {
        code: error.code,
...
export function encodeFrame(frame) {
    ...
    if (body.byteLength > MAX_FRAME_BYTES) {
        throw new ProtocolError("frame_too_large", `frame exceeds maximum size of ${MAX_FRAME_BYTES} bytes`);
    }
 ...
 * @throws {ProtocolError}
 */
export function encodeFrameString(frame) {
    validateFrame(frame, false);
    const json = JSON.stringify(frame);
 ...
 * @throws {ProtocolError}
 */
export function decodeFrameStr(line) {
 ...
 * @throws {ProtocolError}
 */
export function decodeFrameStrStrict(line) {
...
export class FrameDecoder {
        ...
        while (offset < chunk.byteLength) {
            ...
            if (newlineAt === -1) {
                ...
                if (pending > this.maxFrameBytes) {
                    ...
                    throw new ProtocolError("frame_too_large", `frame exceeds maximum size of ${this.maxFrameBytes} bytes`);
                }
            ...
            this.buf = new Uint8Array(0);
            const stripped = lineBytes.byteLength > 0 && lineBytes[lineBytes.byteLength - 1] === 0x0d
    ...
    finishWithFinalLine() {
        ...
        const lineBytes = this.buf.byteLength > 0 && this.buf[this.buf.byteLength - 1] === 0x0d
