...
| G9 | Vendor-neutral telemetry schema and span context | pi-agent | telemetry.rs | TelemetryContext | landed | PAR-TEL (#71) landed pi-agent::telemetry with one injected no-fail TelemetryContext (e91ee94); `AgentLoopConfig::telemetry: Arc<dyn TelemetryContext>` is a required struct field (`crates/pi-agent/src/config.rs:233-234`), so construction without telemetry is compiler-rejected; typed behavior is `TelemetryContext::start_span` (`crates/pi-agent/src/telemetry.rs:57-61`) with `noop_context()` default and `InMemoryTelemetryContext` recorder (`telemetry.rs:213-280`); compaction spans emit through the same injected context. No source-count witness is claimed and no replacement count-based test is asserted as passed. Exporter adapters remain in pi; install-telemetry gating remains C18. |
...
| T1 | Differential rendering engine | pi-tui | frame.rs, terminal/ | frame buffer | landed | Historical regular evidence retained: rendering remains product-agnostic. PAR-PTY-GRILL verified regular-mode: host-tier PTY evidence proves zero full-screen clears, row-local erase with immediate reflow, and continuous content across resizes (crates/pi-tui/tests/pty_grill_adjudication.rs). Fresh fullscreen no-clear/diff evidence is pending DES-07; the old fixture passing result is not fullscreen proof. |
| T2 | Terminal state management | pi-tui | terminal/, link.rs, keys.rs | Terminal capability | landed | Historical regular evidence retained: raw mode, alternate screen, ANSI, OSC, and keyboard protocols share one owner. pi-tui owns terminal capability detection and environment overrides: `TerminalCapabilityOverrides` and `detect_with_overrides` parse `PI_HYPERLINKS` and `PI_TRUE_COLOR` (`1`, `0`, `auto`) before automatic detection. PAR-PTY-GRILL verified regular-mode: probes precede synchronized output, balanced sync markers, restoration on exit, Kitty keyboard flag toggles (crates/pi-tui/tests/pty_grill_adjudication.rs). Fresh fullscreen terminal-mode restoration and resume-size evidence is pending DES-07/DES-09; the old fixture passing result is not fullscreen proof. |
...
| T6 | Layout and widgets | pi-tui | component.rs, components/, layout.rs, overlay.rs, focus.rs | Component and UiEvent | planned | Historical regular evidence retained: the regular event set and product-agnostic component contract are landed. Mouse/fullscreen delta is planned with evidence living in DES-07 per `docs/TUI-G4-supersession.md` (C `9767ba275f3e9a5ee0f5c5342249b629ab1b2282`): the closed `UiEvent` consumer proof is reopened for `UiEvent::Mouse` and native component dispatch. The seven compile-breaking matches requiring explicit disposition are `pi-tui/components/editor/mod.rs::Editor::handle_event`, `pi-ext/adapters.rs::map_ui_event` (Mouse to None), `pi/modes/interactive/runtime.rs::ui_event_wire`, `pi/modes/interactive/runtime.rs::encode_terminal_input` (canonical SGR reconstruction), `pi/modes/interactive/input.rs::InputMapper::map`, `pi-tui/bin/pi_tui_ext_fixture.rs::handle_event`, and `pi-tui/bin/pi_tui_pty_fixture.rs::handle_event`, plus `terminal/input.rs::map_event` becoming `Some(UiEvent::Mouse(event))`. Extension wire intentionally unchanged: no new `UiEventWire` variant, protocol bump, or host mouse branch; extensions observe reconstructed SGR bytes through the existing raw `terminalInput` hook. No pass asserted. |
...
| T9 | Terminal interfaces | pi-tui | terminal/ | Terminal | landed | Historical regular evidence retained: callers never emit terminal escape bytes directly. PAR-PTY-GRILL verified regular-mode: sole stdout owner after probes, transaction markers present, probe batch emitted through terminal interface (crates/pi-tui/tests/pty_grill_adjudication.rs). Fresh fullscreen single-writer evidence is pending DES-07; the old fixture passing result is not fullscreen proof. |
...
The shared arbitration oracle is exactly six `AgentLoopConfig` literal sites: `crates/pi-agent/src/agent.rs:65-93`, `crates/pi-agent/src/config.rs:439-477`, `crates/pi-agent/src/run.rs:1114-1144`, `crates/pi-agent/src/schedule.rs:957-985`, `crates/pi/src/core/agent_session/mod.rs:472-500`, and `crates/pi-agent/src/bin/pi_agent_stream_frame_bench.rs:267-295`.
