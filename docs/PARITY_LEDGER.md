...
| T4 | LaTeX math rendering | pi-tui | components/markdown.rs | markdown math path | planned | Strategy decided in docs/PAR-MATH-latex-strategy.md (issue #36): Rust-native port of the upstream latex.ts layout engine with raw-literal fallback, no JavaScript engine, no new crate; implementation routed to PAR-MATH (issue #37). PAR-PTY-GRILL unverified: ENABLE_MATH not enabled in parser options (markdown.rs:351-354), InlineMath/DisplayMath events silently dropped (markdown.rs:458-459), raw-literal fallback not implemented; open parity gap documented in docs/PAR-PTY-GRILL-verdict.md. |
