...
- Fixture (historical seed, not a re-asserted current capture): `scripts/verification/fixtures/deps-r1-sbom-baseline.json`
  (schema `pi.deps.sbom.v1`, captured 2026-08-27 against the clean tree at
  `20be789`, content sha256 `7a54b9d2bfe2…`).
  The 2026-08-28 re-anchor `849122647411` with content sha256 `c62a6c51aeb2…` is likewise historical; DEPS-R2
  owns the successor ledger. No staged re-capture is claimed here; the owning tool regenerates the fixture after integration.
- **Tooling (historical seed counts):** `scripts/verification/deps-sbom.ts` (`verify:sbom`). Content at the seed = 499 locked Rust packages (48 direct pins, license + direct/dev-only edge position each), toolchain channel + CI pin, three package.json surfaces, both bun.lock files of record (322 + 320 resolutions), the bundled Bun runtime pinned by the `BUN_RUNTIME_VERSION` row of [compatibility.md § Runtime and Release Constants](compatibility.md#runtime-and-release-constants) plus its seven sha256 asset pins, seven release targets.
- Rust package `source` is authoritative: `workspace` for workspace members, `path` for non-member local-path packages (e.g. approved vendored crates wired through `[patch.crates-io]`), and `registry` for all other external sources. The classifier uses cargo metadata's per-package `source` field, not merely workspace membership, so a patched path crate is never misreported as registry.
- `SbomContent.rust.vendorPins` is an optional, omit-when-empty list of `{path, sha256}` file pins for every tracked vendored source / manifest / provenance / license file. A vendored source byte change alters the SBOM digest; the field is absent when no vendored crate is tracked. Vendored coverage is the frozen six-scope `VENDOR_SCOPES` set — `vendor/*/Cargo.toml`, `vendor/*/Cargo.toml.orig`, `vendor/*/LICENSE`, `vendor/*/.cargo_vcs_info.json`, `vendor/*/VENDORED.txt`, `vendor/*/src/**` — where the trailing `**` is required because a Git pathspec wildcard matches the whole path with no implicit directory recursion (`vendor/*/src` matches only the directory entry); crate-local `target/` and `Cargo.lock` stay out of scope as unindexed build residue that is never pinned.
...
- **Reference-refresh law applies:** any commit changing an SBOM input
  (manifest, lockfile, surface, toolchain/CI pin, asset pins, vendored pins) refreshes the
  baseline in the same commit — a red verify means "refresh", never "skip".
- **Committed default is unchanged.** `bun run scripts/verification/deps-sbom.ts capture [--out <file>]` refuses any staged or unstaged dirt over the declared relevant inputs and preserves the existing output. There is no `--allow-dirty-inputs` alias. Committed captures use the private `scripts/verification/capture-inputs.ts` helper for pre/post/publication stability but record no staged provenance. `verify [--snapshot <file>]` takes no `--staged-inputs`; unknown, repeated, or malformed capture flags, a missing `--out` value, a value beginning with another flag, and `--staged-inputs` on `verify` are usage errors before any producer or output mutation.
- **Explicit staged mode (after integration).** `bun run scripts/verification/deps-sbom.ts capture --staged-inputs [--out <file>]` captures only after the helper (`beginStagedInputCapture`, `parseStagedInputProvenance`, `assertCaptureOutputPathsDisjoint`) proves every declared relevant input is an ordinary indexed file with identical raw working bytes. Relevant unstaged content or mode changes, untracked files including ignored-but-untracked files, empty intent-to-add, unmerged entries, symlinks, gitlinks, and skip-worktree or assume-unchanged states all refuse; unrelated staged, unstaged, or untracked changes are ignored. A clean index is legal under this explicit mode for an idempotent refresh; a staged deletion is represented by absence from the inventory (a deleted required manifest then fails the producer normal missing-input check) and a staged rename is legal when the producer can still discover its inputs. Raw-byte equality follows `.gitattributes` (LF-forced, fixtures `-text`); the helper performs no filter, normalization, or object write. The worktree must have an existing commit (detached `HEAD` is valid); repository or index redirection environment variables are rejected rather than captured silently, and both sha1 and sha256 object formats are accepted with case and width checks.
- **Provenance, honestly labelled.** Staged captures add optional `stagedInputProvenance` (`schema: pi.deps.staged-inputs.v1`, `mode: staged`, `baseHead` set to the real current `HEAD` commit, `objectFormat: sha1|sha256`, nonempty sorted `pathspecs`, nonempty sorted `entries` of `{path, mode: 100644|100755, blob}` with each entry inside the declared scope) and set `captureHead` equal to that `baseHead`. `captureHead` names the real parent commit, never the uncommitted result, and is never rewritten to the eventual commit ID. Parsing is offline structural validation; historical objects need not exist locally and `baseHead` need not equal the current `HEAD` for parsing, and malformed stored data is never reordered into validity. When the property is absent, legacy committed behavior is preserved; when present, loaders parse it strictly, require the enclosing `captureHead` to exist and equal `baseHead`, and never fall back to committed mode on null, malformed, or unknown metadata. Schema stays `pi.deps.sbom.v1`, `contentSha256` stays a content-only digest, and provenance is recorded information, not a signature or attestation.
- **Same-green-commit sequence.** Stage only the intended relevant sources (plus the approved dependency and its actual first callsite), run the owning staged captures, stage only the tool-reported generated files, run the gates, then commit together. Generated destinations live outside both input scopes and captures stage nothing. When inputs change after capture, recapture the affected snapshots. This is the supported refresh path without a temporary commit, fake commit ID, expanded dirty exception, install, or manual fixture editing; no intermediate red commit is authorized.
- **Atomic publication, bounded guarantee.** Content is captured in memory, rechecked immediately after capture, serialized and validated, then installed through one atomic file replacement (unique exclusive temporary file in the destination directory, rechecked immediately before `renameSync` over the canonical path; the canonical file is never truncated in place). The pre/post reader with identity and time observations catches ordinary edit-restore races, but it does not prove absence of every adversarial ABA rewrite and is not a global filesystem transaction; no locking, watchers, retries, or transaction framework is claimed. A detected relevant worktree, index, or `HEAD` race stops capture and leaves the old output intact; rerun only when stable. The provenance claim covers exactly the declared indexed inventory; it does not cover ignored `.references`, `node_modules`, hydrated provider catalogs, Cargo home or cache, out-of-repo config, or tool executables and environment.
