...
| [stream-frame-pipeline.md](stream-frame-pipeline.md) | Provider frame decode; assistant state reduction; incremental visible content update | 3 | drain 1,382 ns/frame (fresh, rs 8.91%) | ~0.2 us/frame | ~6.9x | CONSTRAINED-ABOVE-FLOOR (terminal, iteration 33) |
...
| [first-frame-init.md](first-frame-init.md) | Argument parsing + config construction; model/provider construction; TUI construction + layout | 2 | 243.61 ms lane (R2, trusted) | ~1.50 ms | ~162.4x | CONSTRAINED-ABOVE-FLOOR (terminal; post-`9ead528` re-attestation ~118.6 ms ≈ 79x, >=3x minimum preserved at 4.66x cold / 5.06x warm) |
