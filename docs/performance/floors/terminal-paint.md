...
- `stage3_write` performs `write_all` + `flush` per frame (writer.rs:544-573) — one complete write transaction per frame is the observable contract the harness detects (performance.ts frameObservation 1010-1044 keys on the SYNC_END-terminated chunk).
- Consumers: interactive repaint (runtime.rs:4527), stream partial paints (coalescer <=16 ms, runtime.rs:104 BACKGROUND_COALESCE_WINDOW, armed :4523-4524), keypress immediate repaints (runtime.rs:2157-2159).
