...
- Visible update: `handle_partial_update` replaces the streaming assistant view per partial (crates/pi/src/modes/interactive/runtime.rs:2181-2209) behind a <=16 ms coalescer (runtime.rs:4613-4616); the terminal write is coalesced, the event loop keeps every frame.
